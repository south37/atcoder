...
void dfs2(ll v, ll p = -1) {
  for (ll u : g[v]) {
    ...
    // Update u.
    mint now = dp[v];
    ll tot = cnts[v]-1; // decrease 1 as v.

    // Decrease contribution from u.
    now /= c(tot,cnts[u]);
    now /= dp[u];
    tot -= cnts[u];

    // Add contribution from v to u.
    tot += 1;
    ll totU = cnts[u]-1; // decrease 1 as u.
    mint nowU = dp[u];
    totU += tot;
    nowU *= c(totU,tot);
    nowU *= now;
