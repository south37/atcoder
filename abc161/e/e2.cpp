...
int main(int argc, char** argv) {
  ...
  vector<ll> lastL(n, -1); // lastL[i] .. last position before i.
  rep(i, k) {
    if (l[i]+1 < n) { lastL[l[i]+1] = i; }
  }
  rep(i, n-1) {
    if (lastL[i+1] == -1) { lastL[i+1] = lastL[i]; }
  }
  vector<ll> lastR(n, -1); // lastR[i] .. last position after i.
  rep(i, k) {
    if (r[i]-1 >= 0) { lastR[r[i]-1] = i; }
  }
  for (int i = n-1; i > 0; --i) {
  ...
  rep(i, n) {
    ...
    ll ri = lastR[i]; // last position after i.
