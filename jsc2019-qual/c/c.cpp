// ref. https://img.atcoder.jp/jsc2019-qual/editorial.pdf
...
int main(int argc, char** argv) {
  ...
  string s;
  cin >> s;
  if (s[0] == 'W' || s[n-1] == 'W') { // invalid
    cout << 0 << endl;
    return 0;
  }

  // Here, we define the 'LR' vector
  string d(n, ' ');
  d[0] = 'L';
  rep(i, n-1) {
    if (s[i] == s[i+1]) { // d[i] != d[i+1]
      d[i+1] = (d[i] == 'L') ? 'R' : 'L';
    } else { // s[i] != s[i+1], so d[i] == d[i+1]
      d[i+1] = d[i];
    }
  }

  if (d[n-1] == 'L') { // invalid. d[n-1] must be 'R'
    cout << 0 << endl;
    return 0;
  }
  ...
  mint ans(1);
  // Here, scan d.
  ll l = 0; // count of L in [0, i)
  ll r = 0; // count of R in [0, i)
  rep(i, n) {
    if (d[i] == 'L') {
      ++l;
    } else { // d[i] == 'R'
      if (l <= r) { // l is too small. invalid.
        cout << 0 << endl;
        return 0;
      }
      ans *= (l-r);
  ...
  if (l != r) { // inavlid. l must be same with r.
    cout << 0 << endl;
    return 0;
  }
