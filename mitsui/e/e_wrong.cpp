...
int main(int argc, char** argv) {
  ...
  // We check from bottom
  for (int i = n-3; i >= 0; --i) {
    if (a[i] != a[i+1]) {
      // Do nothing. a[i] has asme color with a[i+1];
    } else { // a[i] has different color with a[i+1];
      ans *= 2;
      // if (a[i] == a[i+2]) { // a[i] has different color with a[i+2]
      //   // Do nothing.
      // } else { // a[i] has
      //   p *= 2;
      // }
