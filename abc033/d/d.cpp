// ref. http://mayokoex.hatenablog.com/entry/2016/02/07/201927
...
int main(int argc, char** argv) {
  ...
  rep(i,n) {
    ...
    rep(j,n-1) {
      cnt90    += upper_bound(all(v), v[j]+M_PI/2+eps) - lower_bound(all(v), v[j]+M_PI/2-eps);
      cntLarge += lower_bound(all(v), v[j]+M_PI) - upper_bound(all(v), v[j]+M_PI/2+eps);
    }
  }
