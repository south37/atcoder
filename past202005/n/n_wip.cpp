...
struct LazySegTree { // [L,R)
  ...
  void update(int a, int b, V v, int k, int l, int r) {
    ...
    if (a <= l && r <= b) {
      ...
      dat[k] = comp(dat[k * 2 + 1], dat[k * 2 + 2], k);
    }
  }
  // return index
  ...
  V comp(V l, V r, ll k) { return max(l, r); } // max
...
int main(int argc, char** argv) {
  ...
  rep(iter,q) {
    ...
    if (t == 1) { //
      ...
      // swap x and x+1
      ll tmp = dp.get(x,x+1);
      ll tmp2 = dp.get(x+1,x+2);
      dp.update(x,x+1,tmp2);
      dp.update(x+1,x+2,tmp);
    } else { // t == 2
      ll x,y;
      cin >> x >> y;
      --x; --y;
      dp.setSortFlag(x,y+1);
    }
}
