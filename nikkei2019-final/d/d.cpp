...
struct LazySegTree { // [L,R)
  ...
  void setLazy(int i, V v) { chmax(lazy[i], v); } // add
  void push(int k, int l, int r) {
    chmax(dat[k], lazy[k]); // add
...
int main(int argc, char** argv) {
  ...
  rep(i,m) {
    ll t, l, r;
    tie(t,l,r) = triples[i];
    dp.update(l,r+1,i+1); // update [l,r] by i.
  }
  // Here, we have maximum in each range.
  // we want to know the result in [minL, maxR]
  ll ans = (maxR+1-minL)*maxT;
  for (ll i = minL; i <= maxR; ++i) {
    ll j = dp.get(i, i+1); // max
    ll lastT = 0; // last t at i.
    if (j > 0) {
      lastT = ts[j-1];
    }
    // cout << "i: " << i << endl;
    // cout << "j: " << j << endl;
    // cout << "lastT: " << lastT << endl;
    ans -= (maxT-lastT);
  }
  cout << ans << endl;
}
