...
int main(int argc, char** argv) {
  ...
  auto f = [&](mint xx, ll n) {
    ...
    if (x/MOD != (x+n-1)/MOD) { // Here, multiplies of MOD is in [x,x+n-1]
      return mint(0);
    }
    // Here, x+n-1 < MOD
    return fact[x+n-1] * fact[x-1].invWithGcd();
  };
  ...
  while (q--) {
    ...
    if (d == 0) {
      ...
      cout << f(mint(x),n) << endl;
    } else { // d > 1
      cout << f(mint(x)/mint(d),n) * mint(d).pow(n) << endl;
    }
  }
}
