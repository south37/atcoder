// ref. https://betrue12.hateblo.jp/entry/2020/03/29/232441
...
const int N = 105;
// dp[i][j][k] .. using [0,i], j is prev, k is now
mint dp[N][3][3];
...
int main(int argc, char** argv) {
  ...
  dp[0][0][0] = 1; // initilization.

  rep(i,n) {
    rep(j,3)rep(k,3) {
      rep(now,3) {
        if (i >= 2 && j == k && k == now) { // all are same
          continue;
        }
        if (a[i] >= 0 && a[i] != now) { // previously defined
          continue;
        }
        dp[i+1][k][now] += dp[i][j][k];
      }
    }
  }
