...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i <= min(n-1, k); ++i) {
    // When the count of 0 is i, its contribution is nCi * (n-i)Hi. H is the count of combination with repetition.
    // It is same with nCi * (n-1)C(n-i-1) because xHy = (x+y-1)C(x-1)
